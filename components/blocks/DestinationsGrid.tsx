    ...
    return (
      <section className="bg-white py-20 sm:py-28">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 text-center">
          {content.title && <h2 className="text-3xl sm:text-4xl md:text-[42px] font-bold text-[#0F1A3C]">{content.title}</h2>}
          {content.intro && <p className="mt-4 text-gray-600 text-base sm:text-lg max-w-2xl mx-auto">{content.intro}</p>}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        {(content.title || content.intro) && (
          <div className="text-center mb-14">
            {content.title && <h2 className="text-3xl sm:text-4xl md:text-[42px] font-bold text-[#0F1A3C] leading-tight">{content.title}</h2>}
            {content.intro && <p className="mt-4 text-gray-600 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed">{content.intro}</p>}
          </div>
        )}
              ...
              >
                <div className="relative h-48 overflow-hidden bg-black">
                  {showVideo ? (
                    ...
                    <img src={d.image_url} alt={d.city} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                  ) : (
                ...
                <div className="p-5">
                  ...
                  {d.description && <p className="mt-2 text-gray-600 text-[14px] leading-relaxed line-clamp-2">{d.description}</p>}
