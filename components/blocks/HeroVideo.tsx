...
/**
 * Kaplan TV-style hero with YouTube embed background.
 * Layout matches school-landing-manager Hero.tsx:
 *  - 90vh section, navy overlay (0F1A3C/65)
 *  - 270vh iframe scaled to fill (16:9)
 *  - Badge dot, headline up to 7xl, subtitle, dual CTA (red primary + glass)
 *  - Stats bar at bottom (4 columns over navy/90 backdrop)
 */
export function HeroVideo({ content }: { content: HeroVideoBlock['content'] }) {
  ...
  return (
    <section id="hero" className="relative min-h-[90vh] flex flex-col overflow-hidden">
      ...
      {/* Stats bar — only render if stats provided */}
      {stats.length > 0 && (
